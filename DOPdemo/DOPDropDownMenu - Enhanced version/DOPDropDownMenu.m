...
@property (nonatomic, assign) NSInteger currentSelectedMenudRow;
...
- (void)selectDefalutIndexPath
{
    if (_dataSource && _delegate
        ...
        if (!_isClickHaveItemValid &&[_dataSource respondsToSelector:@selector(menu:numberOfItemsInRow:column:)]
        ...
        } else if ([_dataSource respondsToSelector:@selector(menu:numberOfRowsInColumn:)]
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        if (!self.isClickHaveItemValid && [_dataSource respondsToSelector:@selector(menu:numberOfItemsInRow:column:)] && [_dataSource menu:self numberOfItemsInRow:0 column:i]>0) {
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    if (tapIndex == _currentSelectedMenudIndex && _show) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(menu:numberOfItemsInRow:column:)]) {
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (_dataSource) {
        ...
        for (NSInteger i = 0; i<num;++i) {
            if ([_dataSource respondsToSelector:@selector(menu:numberOfItemsInRow:column:)]
    ...
    if (show) {
        ...
        NSInteger num = [_leftTableView numberOfRowsInSection:0];
        CGFloat tableViewHeight = num * kTableViewCellHeight > 300+1 ? 300:num*kTableViewCellHeight+1;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (_leftTableView == tableView) {
        ...
        if ([self.dataSource respondsToSelector:@selector(menu:numberOfItemsInRow:column:)]) {
            return [self.dataSource menu:self
                    numberOfItemsInRow:self.currentSelectedMenudRow column:self.currentSelectedMenudIndex];
        } else {
            NSAssert(0 == 1, @"required method of dataSource protocol should be implemented");
            return 0;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == _leftTableView) {
        ...
        if ([self.dataSource respondsToSelector:@selector(menu:numberOfItemsInRow:column:)] && [self.dataSource menu:self numberOfItemsInRow:indexPath.row column:_currentSelectedMenudIndex]> 0){
        ...
        cell.backgroundColor = kCellBgColor;//[UIColor whiteColor];
        
    } else {
        if ([self.dataSource respondsToSelector:@selector(menu:titleForItemsInRowAtIndexPath:)]) {
            cell.textLabel.text = [self.dataSource menu:self titleForItemsInRowAtIndexPath:[DOPIndexPath indexPathWithCol:self.currentSelectedMenudIndex row:self.currentSelectedMenudRow item:indexPath.row]];
        } else {
            NSAssert(0 == 1, @"dataSource method needs to be implemented");
        }
        if ([cell.textLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_leftTableView == tableView) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [self.delegate menu:self didSelectRowAtIndexPath:[DOPIndexPath indexPathWithCol:self.currentSelectedMenudIndex row:self.currentSelectedMenudRow item:indexPath.row]];
        } else {
            //TODO: delegate is nil
...
- (BOOL )confiMenuWithSelectRow:(NSInteger)row {
    ...
    if ([self.dataSource respondsToSelector:@selector(menu:numberOfItemsInRow:column:)] && [self.dataSource menu:self numberOfItemsInRow:_currentSelectedMenudRow column:_currentSelectedMenudIndex]> 0) {
...
- (void)confiMenuWithSelectItem:(NSInteger)item {
    ...
    CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex];
    title.string = [self.dataSource menu:self titleForItemsInRowAtIndexPath:[DOPIndexPath indexPathWithCol:self.currentSelectedMenudIndex row:_currentSelectedMenudRow item:item]];
    [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
        _show = NO;
    }];
